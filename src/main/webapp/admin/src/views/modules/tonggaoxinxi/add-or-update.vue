...
export default {
  ...
  methods: {
	...
	this.ruleForm.gonggaoneirong = this.ruleForm.gonggaoneirong.replace(reg,'../../../ssmbqi11/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `tonggaoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tonggaoxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
